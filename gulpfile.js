...
duplicatedSelectors = require('postcss-combine-duplicated-selectors'),
clearComments = require('postcss-discard-comments'),
...
// Concat/minify self-contained scripts or scripts that aren't otherwise used as
// CommonJS type modules.  By default this includes the Foundation script. Other
// possibilities would be polyfill/shim scripts, etc.
...
// site's bundle can `require` from.  If you want to do this add node modules to
// the js.commons.modules array in the yml and uncomment the enqueue for this
// file in functions.php so that WP sends it.
//
// The main advantage is for dev, to reduce the compile time of the bundle task,
// as if the modules are pulled to a commons bundle, they don't have to be
// recompiled when the app bundle changes.
...
gulp.task('js-bundle', function () {
    ...
    var b = browserify(
        {entries: config.js.bundle.entries, debug: true, transform: [babelify]}
    );
...
gulp.task('sass', function () {
    return gulp
        ...
        .pipe(postcss([
          postcssPresetEnv({
            stage: 0,
            browsers: ['last 1 version', 'not dead', '> 0.75%']
          }),
...
// Run a browsersync server, main options managed in gulpconfig.yml Alternative
// to `gulp watch`, run this if you want to make use of browsersync for real
// time code/css updates, etc.
